...
def batch_generator(data_filtered, data_artifacted, time, batch_size):
  ...
  """
  no = len(data_filtered)
  idx = np.random.permutation(no)
  train_idx = idx[:batch_size]     
            
  X_mb = list(data_filtered[i] for i in train_idx)
  T_mb = list(time[i] for i in train_idx)
  Z_mb = list(data_artifacted[i] for i in train_idx)
  return X_mb, T_mb, Z_mb

def batch_generator1(data_filtered, time, batch_size):
  """Mini-batch generator.
