...
import tf_slim
import numpy as np
from utils_v2 import extract_time, rnn_cell, batch_generator, batch_generator1, random_generator
from data_loading import real_data_loading1

def timegan (ori_data_filtered, ori_data_artifacted, parameters):
  """TimeGAN function.
  ...
  # Basic Parameters
  no, seq_len, dim = np.asarray(ori_data_filtered).shape
    
  # Maximum sequence length and each sequence length
  ori_time, max_seq_len = extract_time(ori_data_filtered)
  ...
  # Normalization
  ori_data_filtered, min_val, max_val = MinMaxScaler(ori_data_filtered)  
  ori_data_artifacted, min_val_a, max_val_a = MinMaxScaler(ori_data_artifacted)         
  ...
  def embedder (X, T):
    ...
    with tf.compat.v1.variable_scope("embedder", reuse = tf.compat.v1.AUTO_REUSE):
      ...
      H = tf_slim.fully_connected(e_outputs, hidden_dim, activation_fn=tf.nn.sigmoid)
    return H
  ...
  def recovery (H, T):   
    ...
    with tf.compat.v1.variable_scope("recovery", reuse = tf.compat.v1.AUTO_REUSE):       
      ...
      X_tilde = tf_slim.fully_connected(r_outputs, dim, activation_fn=tf.nn.sigmoid) 
    return X_tilde
  ...
  def generator (Z, T):  
    ...
    with tf.compat.v1.variable_scope("generator", reuse = tf.compat.v1.AUTO_REUSE):
      ...
      E = tf_slim.fully_connected(e_outputs, hidden_dim, activation_fn=tf.nn.sigmoid)     
    return E
  ...
  def supervisor (H, T): 
    ...
    with tf.compat.v1.variable_scope("supervisor", reuse = tf.compat.v1.AUTO_REUSE):
      ...
      S = tf_slim.fully_connected(e_outputs, hidden_dim, activation_fn=tf.nn.sigmoid)     
    return S
  ...
  def discriminator (H, T):
    ...
    with tf.compat.v1.variable_scope("discriminator", reuse = tf.compat.v1.AUTO_REUSE):
      ...
      Y_hat = tf_slim.fully_connected(d_outputs, 1, activation_fn=None) 
    return Y_hat   
  ...
  '''
  tf.compat.v1.train.Saver().restore(sess,'checkpoint2/checkpoint_supervised.ckpt') 
  tf.compat.v1.train.Saver().restore(sess,'checkpoint2/checkpoint_embedding.ckpt')
  tf.compat.v1.train.Saver().restore(sess,'checkpoint2/checkpoint_joint.ckpt') 
  #summary_writer=tf.compat.v1.summary.FileWriter('./log/', sess.graph)
  ...
  for itt in range(iterations):
      # Set mini-batch
   X_mb, T_mb = batch_generator1(ori_data_filtered, ori_time, batch_size)            
     # Train embedder        
   ...
   if itt % 1000 == 0:
     ...
     tf.compat.v1.train.Saver().save(sess,'checkpoint/checkpoint_embedding.ckpt')
   if itt % 10000 == 0:
     tf.compat.v1.train.Saver().save(sess,'checkpoint2/checkpoint_embedding.ckpt')
  ...
  for itt in range(iterations):
      # Set mini-batch
    X_mb, T_mb, Z_mb= batch_generator(ori_data_filtered, ori_data_artifacted, ori_time, batch_size) 
    #X_mb, T_mb = batch_generator1(ori_data_filtered, ori_time, batch_size) 
    ...
    if itt % 1000 == 0:
      ...
      tf.compat.v1.train.Saver().save(sess,'checkpoint/checkpoint_supervised.ckpt')
    if itt % 10000 == 0:
      tf.compat.v1.train.Saver().save(sess,'checkpoint2/checkpoint_supervised.ckpt')
  ...
  for itt in range(iterations):
   ...
   if itt % 1000 == 0:
     ...
     #tf.compat.v1.train.Saver().save(sess,'checkpoint/checkpoint_joint.ckpt')
   if itt % 10000 == 0:
     tf.compat.v1.train.Saver().save(sess,'checkpoint2/checkpoint_joint.ckpt')
  print('Finish Joint Training')

  ## Synthetic data generation
  
  Z_mb = real_data_loading1(max_seq_len)
  #Z_mb = random_generator(no, z_dim, ori_time, max_seq_len)
  generated_data_curr = sess.run(X_hat, feed_dict={Z: Z_mb, X: ori_data_filtered, T: ori_time})    
