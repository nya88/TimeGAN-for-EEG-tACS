...
from utils import train_test_divide, extract_time, batch_generator, batch_generator1
...
def discriminative_score_metrics (ori_data, generated_data):
  ...
  def discriminator (x, t):
    ...
    with tf.variable_scope("discriminator", reuse = tf.AUTO_REUSE) as vs:
      ...
      y_hat_logit = tf_slim.fully_connected(d_last_states, 1, activation_fn=None) 
  ...
  for itt in range(iterations):
    ...
    # Batch setting
    X_mb, T_mb = batch_generator1(train_x, train_t, batch_size)
    X_hat_mb, T_hat_mb = batch_generator1(train_x_hat, train_t_hat, batch_size)
